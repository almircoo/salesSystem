...
<body>
	...
	<div class="container" >
		...
		<c:set var="curso" value="${ registro }" />
		<c:set var="docentes" value="${ listaDocentes }" />
	
		<c:if test="${ curso.getCursoId() == 0 }">
				...
				<div class="col-sm-10" >
					<select name="docenteId" class="form-select" >
						<c:forEach items="${ docentes }" var="d" >
							<c:set var="selected" value="${ d.getDocenteId() == curso.getDocenteId() }" />
							<option value="${ d.getDocenteId() }" ${ selected ? "selected" : "" } >
								${ d.getNombre() }
							</option>
						</c:forEach>
					</select>
